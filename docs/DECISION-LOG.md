...
## 2026-09-06 — Onboarding RC promotion preflight blocked by retained schema drift

- Phil authorised complete accepted dev → main, main push and exact-SHA deployment to the current NUC RC, subject to broader promotion validation. Captured request: `prompts/2026-09-06-onboarding-rc-promotion.txt`. No Rackzar work, data migration/copy/reset, SMTP restoration or DEV/experiment redeployment is authorised.
- Accepted source is `40befbd4687da6af9517d20c2ee8161bfa6c89a0`; main remains `0bc1660f03b8380aedcf24a44881f4196e5eb4de`. Source branches and fetched origin match and are clean. All accepted commits would fast-forward together.
- Live Prisma zero-diff exits 2 because the shared DB retains `CompanyInvitation` (three rows), `InvitationKind` and applied archived migration `0004_company_invitations`. Migration status alone reports up to date. This repeats the documented pre-existing drift; it is not caused by onboarding or this preflight.
- Preserve the table, rows and migration history. The explicit dependency exception does not imply a schema-drift exception. Do not silently restore the rejected archived workflow. Main merge/push and RC deployment remain blocked pending an explicit narrow exception or a separately reviewed, data-preserving reconciliation.
- Keep prompt/evidence on `experiment/onboarding-rc-baseline`, preserving accepted dev/main exactly while blocked. Current NUC surfaces continue sharing realm samma, Mailpit, PostgreSQL and Garage; NUC = DEV, Rackzar = future separate RC/main. See [baseline and validation evidence](ONBOARDING-RC-BASELINE.md).
- Broader validation found two stale supplementary logout assertions. Correct them only on the isolated evidence branch to validate direct provider logout; both corrected registration/Company-intent harnesses pass. Accepted application source and dev/main remain unchanged. Full npm suite passed 79/79 once; current resume/logout and identity/access negatives passed.
- Exact accepted-source typecheck, lint and production build passed. Candidate compiled metadata is RC/main/40befbd. The overlay harness passes desktop/tablet but detects a 390px landing-page Sign in overlap; pointer events still pass through. Record this additional failed check rather than changing the accepted UI silently. No deployment occurred.


## 2026-09-06 — Authorised NUC invitation residue repair and RC promotion continuation

- User authorises disposal only after exact synthetic/archive classification, fresh backup and secret-free forensic export; then a mobile-only badge fix, focused validation, DEV merge/push/deploy and normal main promotion/deployment. Preserve the previous full-suite result and the two validated logout-harness corrections. Request: `prompts/2026-09-06-clear-rc-blockers-and-promote.txt`.
- All invitation parties, companies and connected members/people/records match the archived workflow manifests; no accepted runtime dependency or incoming table reference exists. A backed-up, guarded transaction removed exactly three invitations, CompanyInvitation and InvitationKind. All 23 unrelated tables had identical transaction-local content fingerprints. Linked archived fixture entities and historical migration evidence remain untouched. Prisma validate/status/zero-diff PASS. No new application migration.
...
## 2026-09-07 — Person ↔ Company introduction V1 on DEV

- Phil explicitly authorises direct work/commits/push on `dev`, DEV backup/migration and exact-SHA DEV deployment, overriding the usual experiment-first workflow for this task. Stop after live acceptance; no main promotion. Full request: `prompts/2026-09-07-person-company-introduction-v1.txt`.
- Add only `EmploymentInvitation`; no invitation kind, broad workflow or relationship-column changes. Sending creates invitation metadata and activity only. Verified recipient acceptance creates/activates an EMPLOYMENT relationship and never creates company membership or role grants.
- Bind to a verified existing Account ID when present, always require matching normalized verified email, and recheck current sessions, company state and inviter capability at acceptance. OWNER works through `company.members.manage`; platform Governance privileges alone are insufficient.
- Company/email advisory locking plus Serializable retries provides one open invitation, including concurrent sends and expired re-invites without mutating historical rows. Row locking and Person/company serialization prevent duplicate acceptance. Retry PostgreSQL 40001/40P01 from explicit locks as well as Prisma P2034; return the existing result on an authorised repeat accept.
- Use the authenticated inbox with invitation ID and session-bound HMAC CSRF plus exact Origin checks. Generate 32 random bytes and persist only their SHA-256 hash. Email links to `/person`; the optional bearer-token convenience path is deliberately omitted, so raw tokens never enter URLs or mail.
- Provider-neutral `MailProvider` uses SMTP after database commit. Failure leaves the invitation available in the inbox and reports failed email delivery; the company can revoke/re-invite. No queue/outbox or new workflow. TTL is configurable, set to 24 hours on DEV.
- DEV web joins the existing `samma-mailpit-inbox` network to reach private `samma-mailpit:1025`. No SMTP port publication, Keycloak network/config change, or auth source change. Existing shared NUC PostgreSQL receives one additive table; RC/preview runtimes are not recreated.
- App SMTP uses patched Nodemailer 10.0.0 through the `smtp-mailer` npm alias. This avoids activating Auth.js's unused optional Nodemailer peer, whose supported versions have a current high advisory. No auth provider or dependency version changes; existing production audit gate passes with only the already documented DEV Prisma exception. References: https://nodemailer.com/smtp and https://github.com/nodemailer/nodemailer/security/advisories/GHSA-p6gq-j5cr-w38f .

- Company People displays the accepted invitation address to distinguish connected people whose independent profile still uses a generic display name. It uses company-scoped invitation data, without exposing unrelated Person account details.

### Introduction V1 DEV acceptance
...
- Main local/remote remains `6619d19cddfd0767f280204b541c2f5823f23dce`; RC/preview container IDs/start times and Keycloak/Mailpit containers/networks are unchanged. DEV/RC/preview health/readiness pass. No auth code/provider configuration changes; synthetic registration was exercised normally. See `docs/PERSON-COMPANY-INTRODUCTION-V1-REPORT.md` and its private evidence paths. STOPPED FOR PHIL APPROVAL.
