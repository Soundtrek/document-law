...
export default async function GovernancePage() {
  ...
  return <main className="page-shell"><PageHero eyebrow="SAMMA GOVERNANCE" title="Record policy and platform controls" description="Access is checked against your SAMMA Governance capabilities." nav={governanceNavigation} />
    ...
    <section className="grid"><article className="card full"><h2>Record definitions</h2><Link className="button" href="/governance/definitions">Manage definitions and access matrix</Link>{definitions.length ? definitions.map(definition => <p key={definition.id}>{definition.name} · Version {definition.version} · {definition.classification}</p>) : <p>No record definitions configured.</p>}</article>
