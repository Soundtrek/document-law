...
import { AddTeamMember, TeamInvitationList } from "../../../../components/team-invitations";
import { companyTeamInvitations } from "../../../../lib/team-invitations";
export default async function CompanyTeamPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  return <main className="page-shell"><PageHero eyebrow="COMPANY" title={team.company.name} description={team.viewerRoles.join(", ")} />
    ...
    <section className="card"><h2>Pending invitations</h2>{invitations.length ? <TeamInvitationList company csrf={csrf} invitations={invitations.map(invitation => ({ ...invitation, expiresAt: invitation.expiresAt.toISOString() }))} /> : <p className="muted">No pending team invitations.</p>}</section>
  </main>;
}
