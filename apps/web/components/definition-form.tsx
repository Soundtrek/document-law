...
export function DefinitionForm({ initial, definitionId, expectedVersion, roles, csrf, companyId, base }: {
  ...
  return <form className="definition-form" onSubmit={(event: FormEvent) => { event.preventDefault(); void save("save"); }}>
    <fieldset disabled={busy}><legend>Identity</legend><div className="definition-fields">
      <label className="form-field"><span className="form-label">Name</span><input required maxLength={160} value={policy.name} onChange={e => set("name", e.target.value)} /></label>
      <label className="form-field"><span className="form-label">Stable code</span><input required maxLength={80} pattern={definitionId ? undefined : "[A-Z][A-Z0-9_]*"} readOnly={Boolean(definitionId)} value={policy.code} onChange={e => set("code", e.target.value.toUpperCase())} /><span className="form-help">Choose once. Used for reporting and integrations.</span></label>
      <label className="form-field"><span className="form-label">Category</span><input required maxLength={100} value={policy.category} onChange={e => set("category", e.target.value)} /></label>
      <label className="form-field"><span className="form-label">Description</span><textarea maxLength={2000} value={policy.description} onChange={e => set("description", e.target.value)} /></label>
    </div></fieldset>
    <fieldset disabled={busy}><legend>Direction and Person access</legend><div className="definition-fields">
      <label className="form-field"><span className="form-label">Direction</span><select value={policy.direction} onChange={e => set("direction", e.target.value as DefinitionPolicyInput["direction"])}>{directions.map(d => <option key={d} value={d}>{directionLabel(d)}</option>)}</select></label>
    ...
    <fieldset disabled={busy}><legend>Classification</legend><label className="form-field"><span className="form-label">Classification</span><select value={policy.classification} onChange={e => set("classification", e.target.value as DefinitionPolicyInput["classification"])}>{classifications.map(c => <option key={c}>{c}</option>)}</select></label></fieldset>
    <fieldset disabled={busy}><legend>Review / renewal</legend><label className="definition-check"><input type="checkbox" checked={policy.reviewMonths !== null} onChange={e => set("reviewMonths", e.target.checked ? 1 : null)} />Review required</label>
      {policy.reviewMonths !== null ? <div className="definition-fields"><label className="form-field"><span className="form-label">Review period</span><input required type="number" min={1} max={reviewUnit === "years" ? 1000 : 12000} value={policy.reviewMonths / (reviewUnit === "years" ? 12 : 1)} onChange={e => set("reviewMonths", Number(e.target.value) * (reviewUnit === "years" ? 12 : 1))} /></label><label className="form-field"><span className="form-label">Review unit</span><select value={reviewUnit} onChange={e => { setReviewUnit(e.target.value); set("reviewMonths", 1 * (e.target.value === "years" ? 12 : 1)); }}><option value="months">Months</option><option value="years">Years</option></select></label></div> : <p className="muted">No scheduled review.</p>}
    </fieldset>
    <fieldset disabled={busy}><legend>Retention</legend><label className="form-field"><span className="form-label">Retention policy</span><select value={policy.retentionMode} onChange={e => { const mode = e.target.value as DefinitionPolicyInput["retentionMode"]; setPolicy(p => ({ ...p, retentionMode: mode, retentionMonths: mode === "NONE" ? null : p.retentionMonths ?? 1 })); }}>{retentionModes.map(mode => <option key={mode} value={mode}>{({ NONE: "Not configured", FIXED_FROM_CREATED: "Fixed from created", FIXED_FROM_RELATIONSHIP_END: "Fixed from relationship end" })[mode]}</option>)}</select></label>
      {policy.retentionMode !== "NONE" ? <div className="definition-fields"><label className="form-field"><span className="form-label">Retention period</span><input required type="number" min={1} max={retentionUnit === "years" ? 1000 : 12000} value={(policy.retentionMonths ?? 1) / (retentionUnit === "years" ? 12 : 1)} onChange={e => set("retentionMonths", Number(e.target.value) * (retentionUnit === "years" ? 12 : 1))} /></label><label className="form-field"><span className="form-label">Retention unit</span><select value={retentionUnit} onChange={e => { setRetentionUnit(e.target.value); set("retentionMonths", 1 * (e.target.value === "years" ? 12 : 1)); }}><option value="months">Months</option><option value="years">Years</option></select></label></div> : null}
    ...
    {error ? <p className="form-error notice warning" role="alert">{error}</p> : null}
    <div className="actions"><button className="button" disabled={busy}>{busy ? "Saving…" : definitionId ? "Save as new version" : "Create definition"}</button>
      {definitionId ? <button className={initial.active ? "button danger" : "button secondary"} type="button" disabled={busy} onClick={() => void save(initial.active ? "deactivate" : "activate")}>{initial.active ? "Deactivate" : "Activate"}</button> : null}</div>
  </form>;
}
