...
export async function canReadStoredRecord(db: Database, accountId: string, row: StoredRecord) {
  ...
  return grants.some(grant => Array.isArray(grant.allowedDefinitionIds) && grant.allowedDefinitionIds.every(item => typeof item === "string") &&
